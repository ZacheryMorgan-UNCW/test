        ...
        $.each(data, function(i, v) {
            ...
            eventItem += '</div>';
             if(v.description != undefined){
                eventItem += '<p class="summary">' + text_truncate(v.description, 150) + '</p>';
            }
...
$(".loadView").on("click", function() {
    ...
    clickedView = $(this).text().replace(/All /gi, "").replace(/Upcoming/gi, "Upcoming Events").replace(/Events/gi, "Upcoming Events").replace(/\s/g, "-").replace(/,/g, "").replace(/&/g, "and").replace(/[^0-9a-z\-]/gi, "").toLowerCase();
...
function dateOutput(startDateIn, endDateIn, type) {
    if (moment(startDateIn).isSame(endDateIn, 'day')) {
        console.log("SAME", moment(startDateIn), moment(endDateIn));
        if (type == "full") {
        ...
        console.log("DIFFERENT", moment(startDateIn), moment(endDateIn));
...
function timeOutput(startDateIn, endDateIn, allDayIn, originalStart) {
    if (allDayIn === "true") {
        ...
        if (!moment(startDateIn).utcOffset(stdOffset).isSame(moment(endDateIn).utcOffset(stdOffset))) {
            timeOutput += ' - ';
