/* calendar */
const text_truncate = (str, length = 156, ending = "...") => {
...
$(document).ready(function () {
  ...
  if ($(".events-listing").length) {
    ...
    $("#startDate")
      .datepicker()
      .on("changeDate", function (startDateData) {
    ...
    $("#endDate")
      .datepicker()
      .on("changeDate", function (endDateData) {
        eventsContainer.empty();
        if (startDate != "") {
    ...
    $(".hh-events .plus-toggle").click(function () {
      $(".hh-events .plus-toggle").toggleClass(
  ...
  if ($(".events-detail").length) {
    ...
    if ($(".events-detail .sTime").length) {
      ...
      $(".events-detail .sTime").append(
        timeOutput(
          thisCurrentEvent[0].startDate,
          thisCurrentEvent[0].endDate,
          thisCurrentEvent[0].allDay,
          thisCurrentEvent[0].originalStart
        )
      );
    ...
    if ($(".events-detail .rDate").length) {
      ...
      $("#rDates").change(function (e) {
        ...
        $(".save-row .feeds").append(
          saveLinkOutput(
            $(this).find(":selected").data("title"),
            $(this).find(":selected").data("desc"),
            $(this).find(":selected").data("start"),
            $(this).find(":selected").data("end"),
            $(this).find(":selected").data("timezone"),
            $(this).find(":selected").data("allDay"),
            "google"
          )
        );
        $(".save-row .feeds").append(
...
function eventDateCheck(v, view) {
  if (v.startDate != undefined) {
    if (
      ...
      view === "featured" ||
      view === "side-featured-events" ||
      view === "related-events" ||
      view === "current-recurring-event" ||
      view === "hh-events-block"
    ) {
      // if (moment(v.startDate).isSameOrAfter(moment(), 'day') && moment(v.endDate).isSameOrAfter(moment())) {
      if (moment(v.endDate).isSameOrAfter(moment())) {
      ...
      if (
        moment(moment(today).add(1, "days")).isSame(moment(v.startDate), "date")
      ) {
        checkedEvents.push(v);
      }
    } else if (view === "this-week") {
      if (
        moment(moment(v.startDate)).isSameOrAfter(moment().weekday(0), "day") &&
        moment(moment(v.startDate)).isSameOrBefore(moment().weekday(6), "day")
      ) {
        checkedEvents.push(v);
      }
    } else if (view === "this-weekend") {
      if (
        moment(moment(v.startDate)).isSameOrAfter(moment().weekday(5), "day") &&
        moment(moment(v.startDate)).isSameOrBefore(moment().weekday(7), "day")
      ) {
        checkedEvents.push(v);
      }
    } else if (view === "next-week") {
      if (
        moment(moment(v.startDate)).isSameOrAfter(moment().weekday(7), "day") &&
        moment(moment(v.startDate)).isSameOrBefore(moment().weekday(13), "day")
      ) {
        checkedEvents.push(v);
      }
      ...
      if (view === undefined || view == "") {
        ...
        if (view.indexOf("-") > -1) {
          if (
            moment(v.startDate).isSameOrAfter(
              moment(rangeDateStart, "MM/DD/YYYY")
            ) &&
            moment(v.startDate).isSameOrBefore(
              moment(rangeDateEnd, "MM/DD/YYYY")
            )
          ) {
            checkedEvents.push(v);
          }
...
function eventCount() {
  ...
  $(eventFilterViewCountDisplay).empty();
  if (
    $(".events-list .row.event").length !=
    $(".events-list .filter-match").length
  ) {
    $(eventViewCountDisplay).html(
      "Loaded <span>" + $(".events-list .row.event").length + "</span> event(s)"
    );
    $(eventFilterViewCountDisplay).html(
      ", showing <span>" +
        $(".events-list .filter-match").length +
        "</span> that meet your filter/search criteria. "
    );
  } else {
    $(eventViewCountDisplay).html(
      "Showing <span>" +
  ...
  if (
    $(".events-list .row.event").length <= 4 ||
    $(".events-list .filter-match").length <= 4
  ) {
...
function eventHTML(v, col) {
  ...
  if (v.recurringEvent == "true") {
    ...
    eventItem +=
      '<span class="fas fa-sync-alt" title="' +
      v.tooltip +
  ...
  eventItem +=
    '<div class="time">' +
    timeOutput(v.startDate, v.endDate, v.allDay, v.originalStart) +
    "</div>";
  eventItem +=
    '<div class="date">' + dateOutput(v.startDate, v.endDate) + "</div>";
  eventItem += "</div>";
  eventItem += "</div>";
  return eventItem;
}
...
function getEvents(data, showLimit, origin) {
  return new Promise(function (resolve, reject) {
    ...
    $.each(data, function (i, v) {
      ...
      eventItem = "";
      eventItem +=
        '<div class="row event filter-match ' + filterClasses.join(" ") + '">';
      eventItem += '<a href="' + v.url + '" target="_parent">';
      eventItem += '<div class="col-xs-12 col-sm-4 image">';
      eventItem += imageOutput(v.image, v.startDate);
      eventItem += "</div>";
      eventItem +=
        '<div class="col-xs-12 col-sm-8"><div class="event-details"><div>';
      eventItem += '<h3 class="title">';
      eventItem += v.title;
      eventItem += "</h3>";
      eventItem += '<div class="details">';
      ...
      if (eventsListFilterTexts.length > 0) {
        if (eventsListFilterTexts.length > 1) {
          eventItem +=
            '<div class="location listFilterText">' +
      ...
      if (eventsListFilterDots.length > 0) {
        ...
        $.each(eventsListFilterDots, function (i, v) {
          eventItem +=
            ...
            '-color" title="' +
            v.name +
            '" data-toggle="tooltip" data-placement="bottom" tabindex="0"></span>';
        });
        eventItem += "</div>";
      }
      eventItem += "</div>";
      if (v.description != undefined) {
        eventItem +=
          '<p class="summary">' + text_truncate(v.description, 150) + "</p>";
      }
        ...
        if (!_.includes(window.location.hash, hashChecked)) {
          ...
          getHash(window.location.hash, hashChecked, function () {
            setHash(hashChecked, "add", thisChecked, function () {
              showFilter(e, setFiltersLoad, "loadView 1");
            });
          });
          ...
          if (_.includes(window.location.hash, thisChecked)) {
            if (debugMode) {
              console.log(
                "filter remove: " + thisChecked + " from " + hashChecked
              );
            }
            getHash(window.location.hash, hashChecked, function () {
              setHash(hashChecked, "remove", thisChecked, function () {
                showFilter(e, setFiltersLoad, "loadView 3");
              });
            });
          } else {
            if (debugMode) {
            ...
            getHash(window.location.hash, hashChecked, function () {
              setHash(hashChecked, "add", thisChecked, function () {
                showFilter(e, setFiltersLoad, "loadView 2");
              });
            });
          }
        }
...
$("#search").on("keydown", function (e) {
  if (e.keyCode == 8) {
    loadView(currentLoadedView, "", "all", "searchText").then(function (
      results
    ) {
      showEvents(
        results.events,
...
function searchText(textSearch) {
  ...
  if (textSearch.length > 0) {
    $(eventsContainer)
      ...
      .each(function (e) {
        if (
          !$(this).find(".title").text().toLowerCase().match(textSearch) &&
          !$(this).find(".details").text().toLowerCase().match(textSearch) &&
          !$(this).find(".summary").text().toLowerCase().match(textSearch)
        ) {
...
function updateDropdowns(classList) {
  let filter;
  if (classList.includes("filter4")) {
    filter = "4";
  } else if (classList.includes("filter3")) {
    filter = "3";
  } else if (classList.includes("filter2")) {
    filter = "2";
  }
  // const filterString = `#filter${filter}-mdp`
  const checkedFilterInputs = document.querySelectorAll(
    `.loadFilter.filter${filter}:checked`
  );
  const parentTexts = Array.from(checkedFilterInputs).map((input) =>
    input.parentElement.innerText.trim()
  );
  const filterDropdown = document.querySelector(`#filter${filter}-mdp`);
  if (checkedFilterInputs.length == 0) {
    filterDropdown.innerText = "Select";
  } else if (checkedFilterInputs.length == 1) {
    filterDropdown.innerText = parentTexts[0];
  } else if (checkedFilterInputs.length > 1) {
    filterDropdown.innerText =
      parentTexts[0] + ", (" + checkedFilterInputs.length + ")";
  }
}

function showFilter(e, filter, origin) {
  if (e?.target?.classList) {
    const classList = [...e.target.classList];
    console.log(classList);
    updateDropdowns(classList);
  }
  // for (let i = 2; i <= 4; i++) {
  //   const checkedFilterInputs = document.querySelectorAll(
  //     `.loadFilter.filter${i}:checked`
  //   );
  //   updateDropdowns(checkedFilterInputs, `#filter${i}-mdp`);
  // }
...
function checkFilter(filter) {
  ...
  $.each(checkFilters, function (i, checkFiltersValue) {
    $(".loadFilter:checkbox[value=" + checkFiltersValue + "]").prop(
      "checked",
      "true"
    );
  });
...
function calculateRecurrences(data) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    $.each(data, function (i, event) {
      if (event != undefined && event.recurringEvent === "true") {
        /* Get the date data */
        ...
        var untilMoment = moment.parseZone(
          event.recurringDetails.recurUntilDate
        );
        ...
        if (event.recurringDetails.recurFrequency === "daily") {
          if (event.recurringDetails.recurInterval > 1) {
            event.tooltip =
              "Every " + event.recurringDetails.recurInterval + " days";
          } else {
            event.tooltip = "Daily";
          }
          while (untilMoment.diff(startMoment, "s") >= 0) {
            ...
            /* Increment by X days */
            startMoment
              .add(event.recurringDetails.recurInterval, "days")
              .valueOf();
            endMoment
              .add(event.recurringDetails.recurInterval, "days")
              .valueOf();
          ...
          if (event.recurringDetails.recurInterval > 1) {
            event.tooltip =
              "Every " + event.recurringDetails.recurInterval + " weeks";
          } else {
            event.tooltip = "Weekly";
          }
          if (
            event.recurringDetails.recurDays != undefined &&
            event.recurringDetails.recurDays.length > 0
          ) {
            event.tooltip += " on ";
            for (
              var day = 0;
              day < event.recurringDetails.recurDays.length;
              day++
            ) {
            ...
            while (untilMoment.diff(startMoment, "s") >= 0) {
              /* Loop over the selected days of the week */
              for (
                var day = 0;
                day < event.recurringDetails.recurDays.length;
                day++
                ...
                startMoment.date(
                  startMoment.date() +
                    (daysArray.indexOf(thisDay) - startMoment.day())
                );
                ...
                if (startMoment.diff(_startMoment, "s") >= 0) {
                  ...
                  thisRecurrence.endDate = endMoment.valueOf();
                  if (
                    !checkCancellations(cancellations, thisRecurrence.startDate)
                  ) {
                    recurringData.push(thisRecurrence);
                  }
                }
            ...
            while (untilMoment.diff(startMoment, "s") >= 0) {
              ...
              /* Increment by X weeks */
              startMoment
                .add(event.recurringDetails.recurInterval, "weeks")
                .valueOf();
              endMoment
                .add(event.recurringDetails.recurInterval, "weeks")
                .valueOf();
          ...
          if (event.recurringDetails.recurInterval > 1) {
            event.tooltip =
              "Every " + event.recurringDetails.recurInterval + " months";
          } else {
            event.tooltip = "Monthly";
          }
          if (event.recurringDetails.recurMonth === "day of the month") {
            /* The same day of the month on each month (1st, 2nd, etc) */
          ...
          } else if (event.recurringDetails.recurMonth === "day of the week") {
            /* The same day of the week of the month (2nd Tues of month) */
            event.tooltip +=
              " on the " +
              nth(Math.ceil(startMoment.date() / 7)) +
              " " +
              fullDayArray[
                startMoment.day()
            ...
            while (untilMoment.diff(startMoment, "s") >= 0) {
              ...
              startMoment
                .add(event.recurringDetails.recurInterval, "months")
                .date(1);
              tempMonth = startMoment.month();
              if (theDay <= startMoment.day()) {
                startMoment.date(
                  theDay - startMoment.day() + (theWeek + 1) * 7
                );
              } else {
          ...
          if (event.recurringDetails.recurInterval > 1) {
            event.tooltip =
              "Every " + event.recurringDetails.recurInterval + " years";
          } else {
          ...
          while (untilMoment.diff(startMoment, "s") >= 0) {
            ...
            startMoment
              .add(event.recurringDetails.recurInterval, "years")
              .valueOf();
            endMoment
              .add(event.recurringDetails.recurInterval, "years")
              .valueOf();
...
$(".loadView").on("click", function () {
  ...
  /*If changing buttons for the view to use label you will need the following code*/
  clickedView = $(this)
    .text()
    .replace(/All /gi, "")
    .replace(/Upcoming/gi, "Upcoming Events")
    .replace(/Events/gi, "Upcoming Events")
    .replace(/\s/g, "-")
    .replace(/,/g, "")
    .replace(/&/g, "and")
    .replace(/[^0-9a-z\-]/gi, "")
  ...
  return new Promise(function (resolve, reject) {
    ...
    viewChecker(view).then(function (results) {
      if (view === "upcoming-events") {
        $(eventViewHeader).html("All Events");
        // 			checkDataURLs(results, view, target, showLimit, origin);
        checkDataURLs(results, view, target, showLimit, origin).then(function (
          results
        ) {
        ...
        // Changes button to active if the view on load matches
        $(".date-views button:contains('All Events')").addClass("active");
      } else if (view === "this-month") {
        ...
        // Changes button to active if the view on load matches
        $(".date-views button:contains('This Month')").addClass("active");
      } else if (view === "today") {
        ...
        // Changes button to active if the view on load matches
        $(".date-views button:contains('Tomorrow')").addClass("active");
      } else if (view === "this-week") {
        ...
        // Changes button to active if the view on load matches
        $(".date-views button:contains('This Weekend')").addClass("active");
      } else if (view === "next-week") {
        ...
        $(eventViewHeader).html("Featured Events");
        checkDataURLs(results, view, target, showLimit, origin).then(function (
          results
        ) {
          resolve({
        ...
        // Changes button to active if the view on load matches
        $(".date-views button:contains('Featured')").addClass("active");
      } else if (view === "side-featured-events") {
        checkDataURLs(results, view, target, "3", origin).then(function (
          results
        ) {
          resolve({
            events: results.events,
  ...
  return new Promise(function (resolve, reject) {
    if (
      ...
      if (view.indexOf("-") > -1) {
        ...
        if (
          !moment(rangeDateStart, "MM/DD/YYYY", true).isValid() &&
          !moment(rangeDateEnd, "MM/DD/YYYY", true).isValid()
        ) {
          eventsMessage =
            rangeDateStart +
            "-" +
            rangeDateStart +
            " is not a valid date range.";
          errorMessage = true;
        } else {
        ...
        if (errorMessage) {
          ...
          if (startYearRange == endYearRange) {
            if (startMonthRange == endMonthRange) {
              if (
                currentYear === startYearRange &&
                startMonthRange >= previousMonth
              ) {
                currentDataURLs.push(currentDataURL.replace(/&amp;/g, "&"));
              } else {
              ...
              if (
                currentYear === endYearRange &&
                endMonthRange >= previousMonth
              ) {
                currentDataURLs.push(currentDataURL.replace(/&amp;/g, "&"));
              } else {
                dataFiles.forEach(function (dF) {
        ...
        if (!errorMessage) {
          ...
          if (
            currentYear === singleDateYear &&
            singleDateMonth >= previousMonth
          ) {
            currentDataURLs.push(currentDataURL.replace(/&amp;/g, "&"));
          } else {
            dataFiles.forEach(function (dF) {
...
function saveLinkOutput(
  ...
  if (calendar == "google") {
    saveLinkOutput +=
      "<a href='https://calendar.google.com/calendar/r/eventedit?text=" +
      urlText(title) +
      "&amp;dates=" +
      ssd +
      "/" +
      ...
      " ' target='_blank'>";
    saveLinkOutput +=
      "<span class='fab fa-google' aria-hidden='true'></span> <span class='sr-only'>Add to</span>Google</a>";
  } else if (calendar == "outlook") {
    saveLinkOutput +=
      "<a target='_blank' href='https://outlook.live.com/owa/?path=/calendar/action/compose&amp;startdt=" +
      ssd +
      "&amp;enddt=" +
      sed +
      "&amp;subject=" +
      ...
      " ' target='_blank'>";
    saveLinkOutput +=
      "<span class='fab fa-windows' aria-hidden='true'></span> <span class='sr-only'>Add to</span>Outlook</a>";
  } else if (calendar == "yahoo") {
    if (allDay == "true") {
      saveLinkOutput +=
        "<a href='https://calendar.yahoo.com/?v=60&amp;view=d&amp;type=20&amp;title=" +
        urlText(title) +
        "&amp;st=" +
        ssd +
        "&amp;et=" +
    ...
    saveLinkOutput +=
      "<span class='fab fa-yahoo' aria-hidden='true'></span> <span class='sr-only'>Add to</span>Yahoo</a>";
  } else if (calendar == "ics") {
    saveLinkOutput +=
      "<a href='" +
      thisCurrentEvent[0].ics +
      "' target='_blank'><span class='far fa-calendar-plus' aria-hidden='true'></span> <span class='sr-only'>Download as</span>iCal</a>";
  }
...
function currentRecurringEvent() {
  calculateRecurrences(thisCurrentEvent).then(function (resultsR) {
    eventDateChecker("", resultsR, "current-recurring-event").then(function (
      results
    ) {
      jsonToCurrentData(results).then(function (results) {
        results.jsonToCurrentDataEvents.sort(function (a, b) {
        ...
        if (results.jsonToCurrentDataEvents.length > 0) {
          $.each(results.jsonToCurrentDataEvents, function (i, v) {
            $(".rDate").append(
              "Next Occurrance:<br/>" +
                dateOutput(v.startDate, v.endDate, "full")
            );
            $("#rDates").append(
          ...
          $(".save-row").remove();
          $(".rDate").append(
            "This event has ended. It ran from<br/>" +
              moment(thisCurrentEvent[0].startDate).format("MMM D, YYYY") +
              " to " +
              moment(
                thisCurrentEvent[0].recurringDetails.recurUntilDate
              ).format("MMM D, YYYY") +
              "."
          );
        }
  ...
  if (moment(startDateIn).isSame(endDateIn, "day")) {
    ...
    if (moment(startDateIn).isSame(endDateIn, "year")) {
      if (moment(startDateIn).isSame(endDateIn, "month")) {
        var dateOutput =
          moment(startDateIn).format("MMM D") +
          " - " +
          moment(endDateIn).format("D YYYY");
      } else {
        var dateOutput =
          moment(startDateIn).format("MMM D") +
          " - " +
          moment(endDateIn).format("MMM D YYYY");
      }
    } else {
      var dateOutput =
        moment(startDateIn).format("MMM D YYYY") +
        " - " +
        moment(endDateIn).format("MMM D YYYY");
    }
  }
...
function imageOutput(image, startDateIn) {
  if (image) {
    ...
    var imageOuput =
      '<div class="event-image date-img"><div class="month">' +
...
function timeOutput(startDateIn, endDateIn, allDayIn, originalStart) {
  if (allDayIn === "true") {
    ...
    if (
      moment(startDateIn).utcOffset(stdOffset).format("a") !=
      moment(endDateIn).utcOffset(stdOffset).format("A")
    ) {
    ...
    if (
      !moment(startDateIn)
        .utcOffset(stdOffset)
        .isSame(moment(endDateIn).utcOffset(stdOffset))
    ) {
      timeOutput += " - ";
        ...
        if (fT) {
          ...
          if (Array.isArray(eventFilter)) {
            eventFilter.forEach(function (ef, i) {
              eventFilterCheck.push(
                ef.toLowerCase().replace(/[^A-Za-z0-9]/g, "")
              );
            });
  ...
  $.ajax({
    ...
    success: function (URLData) {
      calculateRecurrences(URLData.events).then(function (resultsR) {
        eventDateChecker(URLData.events, resultsR, "related-events").then(
          function (results) {
            jsonToCurrentData(results).then(function (results) {
              results.jsonToCurrentDataEvents.sort(function (a, b) {
                return +new Date(a.startDate) - +new Date(b.startDate);
              ...
              if (relatedEvents.length > 0) {
                ...
                $(".related-events .events").empty();
                $(".related-events .events").append(
                  '<div><p style="text-align:center">No upcoming related events found.</p></div>'
                );
              }
            });
          }
...
function blockEvents() {
  $.ajax({
    ...
    success: function (URLData) {
      calculateRecurrences(URLData.events).then(function (resultsR) {
        eventDateChecker(URLData.events, resultsR, "hh-events-block").then(
          function (results) {
            jsonToCurrentData(results).then(function (results) {
              results.jsonToCurrentDataEvents.sort(function (a, b) {
                return +new Date(a.startDate) - +new Date(b.startDate);
              ...
              if (hhEvents.length > 0) {
                ...
                $(".hh-events-block .row.events").empty();
                $(".hh-events-block .row.events").append(
                  '<div><p style="text-align:center">No upcoming events found.</p></div>'
                );
              }
            });
          }
